...
HDS Interlude（HDSI）的效果并不只是因为“人设写得更细”。它本身是一个独立的多层运行框架：一次在用户看来只是收发几句话的互动，内部可能先经过消息合并与中断处理、当前事件和时钟锚定、场景、关系与记忆读取，再由主叙事模型补写已经经过的时间、处理当前事件，并决定回复、沉默或延迟。视觉理解、记忆压缩、情绪偏移和日程审查等职责还可以在需要时交给独立的侧端模型调用，最后再由宿主保存状态、调度动作并负责真实投递。

因此，HDSI 展示出的自然感是**多层状态、主模型与按需侧端模型、宿主调度共同产生的框架效果**，不是某一张角色 Prompt 单独“演得好”。主模型不必在一次可见回答里同时证明自己理解了全部上下文、维护了长期事实、处理了图片、安排了未来动作并完成了情绪服务；复杂过程可以留在系统内部，用户最终只看到经过选择后真正需要发送的互动。
...
但 HDSI 是需要独立部署的完整框架，而当前主流 LLM RP 环境通常仍是“一个主模型 + 一份角色 Prompt + 通用聊天宿主”。本仓库不要求用户复现 HDSI，也没有移植它的代码、固定 Prompt、字段、JSON 协议、阶段流程或持续世界模拟，更不提供它的运行时保证。

我们选择的是更窄、也更容易迁移的路径：在模型、宿主和可用动作都不改变时，通过 Skill 学习 HDSI 中可蒸馏的职责分离机制，再把它们编译成一份统一角色 Prompt。它主要改善单个主模型的扮演层，让模型更清楚地区分角色定义与动态信息、内部理解与可见表达、当前事件与历史延续、角色自身动机与任务式交付。它不能凭空增加侧端模型、持久状态、真实时间、调度或投递能力，也不等价于用一张 Prompt 复现 HDSI。
