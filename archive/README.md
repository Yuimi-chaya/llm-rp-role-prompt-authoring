...
本目录保留历史与实验性作者文件，用于复现和迁移。

`archive/` 下没有当前 Skill 入口。不要把归档文件与 `skills/role-prompt-authoring/` 混合，也不要让自动 Skill 路由把它们当作当前替代方案。
...
Nothing under `archive/` is a current Skill entry point. Do not combine archived files with `skills/role-prompt-authoring/`, and do not let automatic Skill selection treat them as alternatives.
